...
function viewRequest(event){
				...
				fetch(API_PREFIX+'/api/v2/requests/'+event.cells[0].innerHTML, {
					method: 'GET',
					headers:{
						"Accept":"application/json",
						"Content-type":"application/json",
				...
				searchTitle = document.getElementById('filterbystatus');
				searchTitle.addEventListener('keyup', filterStatus);
			
				function filterStatus() {
					let filterValue = document.getElementById('filterbystatus').value.toUpperCase();
			
					// Get table rows
					let tr = document.getElementsByClassName('status');
					// Get entries
